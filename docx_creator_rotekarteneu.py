    ...
    def create(self):
        for row_count, row in self.__tours.iterrows():
            ...
            if 'SeniorenA' in groups:
                self.__add_line(row, self.__document_seniorenA)
            if 'SeniorenB' in groups:
                self.__add_line(row, self.__document_seniorenB)
            if 'SeniorenC' in groups:
                self.__add_line(row, self.__document_seniorenC)
    ...
    def __add_line(self, row, document):
        if (self.__is_whole_day(row)):
            line = '{0}\t{1}\t{2}\t{3} / {4}\t{5}'.format(
                ...
                self.__get_activity(row),
                self.__get_type(row),
                self.__get_requirements(row),
                self.__get_guide(row)
            )
        else:
            line = '{0}\t{1}, {2}\t{3} / {4}\t{5}'.format(
                self.__get_date(row),
